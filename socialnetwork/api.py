...
def similar_users(user: SocialNetworkUsers):
    ...
    # get user expertise and fame values
    user_fames = Fame.objects.filter(user=user, fame_level__isnull=False).select_related('fame_level')
    user_expertise = {f.expertise_area_id: f.fame_level.numeric_value for f in user_fames}
    num_expertise = len(user_expertise)
    
    if num_expertise == 0:
        return []

    # fetch other users and their fame values
    others = list(user.__class__.objects.exclude(pk=user.pk))
    other_fames_map = {}
    
    all_fames = Fame.objects.filter(user__in=others, fame_level__isnull=False).select_related('fame_level')
    for f in all_fames:
        other_fames_map.setdefault(f.user_id, {})[f.expertise_area_id] = f.fame_level.numeric_value

    # calculate similarity score for each user
    results = []
    
    for other in others:
        other_fames = other_fames_map.get(other.pk, {})
        score_sum = 0
        
        for area, user_fame_val in user_expertise.items():
            other_fame_val = other_fames.get(area, float('inf'))
            
            if abs(user_fame_val - other_fame_val) <= 100:
                score_sum += 1
                
        score = score_sum / num_expertise
        
        if score > 0.0:
            other.similarity = score
    ...
    for i in range(n):
        for j in range(0, n - i - 1):
            u1, u2 = results[j], results[j + 1]
            swap = False
            
            if u1.similarity < u2.similarity:
                swap = True
            elif u1.similarity == u2.similarity:
                if u1.date_joined < u2.date_joined:
                    swap = True
                elif u1.date_joined == u2.date_joined and u1.pk < u2.pk:
                    swap = True
                    
            if swap:
                results[j], results[j + 1] = u2, u1
